...
const validatePaymentRequest = (request, userId, planType, amount) => {
    ...
    if (!Number.isFinite(numAmount) || numAmount !== planConfig.amount) {
        throw new functions.https.HttpsError("invalid-argument", `Invalid amount for ${planType} plan.`);
    }
 ...
 */
exports.createRazorpayOrder = functions.https.onCall(callableOptions, async (request) => {
    try {
 ...
 */
exports.verifyRazorpayPayment = functions.https.onCall(callableOptions, async (request) => {
    try {
